...
class Supervised_TM:
    ...
    def extract__features(self, documents, is_training=True):
        ...
        if is_training:
            self.embedding_reducer = TruncatedSVD(
                n_components=50, random_state=self.random_state)
            reduced_embeddings = self.embedding_reducer.fit_transform(
                embeddings)
        else:
    ...
    def _compute_document_topic_similarities(self, documents):
        ...
        try:
            ...
            all_topics = sorted(
                [t for t in self.topic_model.get_topics().keys() if t != -1])
            ...
            for topic_id in all_topics:
                try:
                    ...
                    topic_words = [word for word, score in self.topic_model.get_topic(topic_id)[
                        :10]]
                    if topic_words:
                        ...
                        topic_centroids.append(
                            np.zeros(doc_embeddings.shape[1]))
            ...
            print(
                f"Document-topic similarity features shape: {similarities.shape}")
            return similarities
        ...
        for doc in documents:
            ...
            doc_features = [
                ...
                np.mean([len(word) for word in words]
                        ) if words else 0,  # Average word length
            ]
    ...
    def predict(self, documents):
        ...
        try:
            ...
            if self.feature_selector is not None:
                features_selected = self.feature_selector.transform(
                    features_scaled)
            else:
    ...
    def load_news(self, path):
        ...
        df.columns = ['news_id', 'category', 'subcategory', 'title',
    ...
    def init(self):
        ...
        train_df = self.load_news("Mind_train")
        val_df = self.load_news("Mind_val")
        ...
        print(
            f"Number of unique categories: {len(self.label_encoder.classes_)}")
        ...
        unique_labels = sorted(
            list(set(y_test + results['predictions'].tolist())))
        target_names_filtered = [
            self.label_encoder.classes_[i] for i in unique_labels]
...
#     model_example.init()
#     example = ["This is a sample news article about technology and innovation.",
#                "A new breakthrough in AI technology has been achieved.",
#                "The stock market is experiencing significant fluctuations today."]
...
def main():
    ...
    # Get total count of documents without category
    total_docs = collection.count_documents({"category": {"$exists": False}})

    pbar = tqdm(total=total_docs, desc="Processing articles", unit="docs")

    try:
        while pbar.n < total_docs:
            # Fetch batch of documents without category
            ...
            doc_ids = [doc["_id"] for doc in docs if doc.get("title")]

            if titles:
                # Get predictions
                predictions = model.u_predict(titles)
                ...
                bulk_ops = [
                    ...
                    for doc_id, prediction in zip(doc_ids, predictions)
                ]

                result = collection.bulk_write(bulk_ops)
                pbar.update(result.modified_count)
            else:
                pbar.update(len(docs))  # Skip docs without titles
        ...
        pbar.close()
        client.close()
        print(f"Completed. Processed {pbar.n} documents.")
