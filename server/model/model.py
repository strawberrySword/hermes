...
def parse_behaviors(file_path):
    ...
    interaction_matrix = csr_matrix(
        (data, (rows, cols)), shape=(user_counter, item_counter))
...
def get_recommendations(user_id, num_recommendations=5) -> list[str]:
    recommendations = recommend(
        user_id, user_factors, item_factors, user_map, item_map, reverse_item_map, user_clicks, num=num_recommendations
    )
    return recommendations
